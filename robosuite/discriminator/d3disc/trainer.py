"""D3-Disc dynamics trainer — latent-only, predictor-only.

Unlike ``lpb.trainer.Trainer`` which also forwards images through a ResNet
encoder each step, we already have z_t cached on disk (flow_multi encoder is
frozen). This trainer feeds (z_t, s_t, a_chunk) to the ``DynamicsPredictor``
and regresses ``(z_{t+h}, s_{t+h})`` directly.
"""
...
class D3DynamicsTrainer:
    """Train the latent dynamics predictor on cached flow_multi latents."""
    ...
    def __init__(
        ...
        config: Optional[D3TrainerConfig] = None,
        device: Optional[str] = None,
    ) -> None:
        self.predictor = predictor
        self.train_dataset = train_dataset
        self.val_dataset = val_dataset
        self.cfg = config or D3TrainerConfig()
    ...
    def _resolve_expert_labels(self, dataset: Dataset) -> Optional[List[bool]]:
        if isinstance(dataset, LatentFlowDynamicsDataset):
            return dataset.sample_is_expert
        if isinstance(dataset, Subset) and isinstance(dataset.dataset, LatentFlowDynamicsDataset):
    ...
    def _build_train_loader(self) -> DataLoader:
        ...
        if use_balanced:
            ratio = min(max(float(self.cfg.expert_sampling_ratio), 0.0), 1.0)
    ...
    def _run_step(self, batch: Dict[str, torch.Tensor], train: bool) -> Dict[str, float]:
        ...
        with torch.set_grad_enabled(train):
            pred = self.predictor(
                obs_token=b["current_latent"],
                proprio_token=b["current_proprio"],
                action_tokens=b["action_sequence"],
            )
            latent_mse = F.mse_loss(pred["pred_latent"], b["target_latent"])
            proprio_mse = F.mse_loss(pred["pred_proprio"], b["target_proprio"])
            loss = latent_mse + float(self.cfg.proprio_loss_weight) * proprio_mse

            if train:
                loss.backward()
                if self.cfg.grad_clip_norm > 0:
                    torch.nn.utils.clip_grad_norm_(
                        self.predictor.parameters(), self.cfg.grad_clip_norm
                    )
                self.optimizer.step()
    ...
    def train_one_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for step, batch in enumerate(self.train_loader):
            ...
            if self.cfg.log_every > 0 and step % self.cfg.log_every == 0:
                print(
                    f"[d3_train] epoch={epoch:03d} step={step:05d} "
                    f"loss={out['loss']:.6f}  z_mse={out['latent_mse']:.6f}  "
                    f"s_mse={out['proprio_mse']:.6f}"
                )
    ...
    def validate(self, epoch: int) -> Dict[str, float]:
        ...
        for step, batch in enumerate(self.val_loader):
            ...
            if self.cfg.log_every > 0 and step % self.cfg.log_every == 0:
                print(
                    f"[d3_valid] epoch={epoch:03d} step={step:05d} "
                    f"loss={out['loss']:.6f}  z_mse={out['latent_mse']:.6f}  "
                    f"s_mse={out['proprio_mse']:.6f}"
                )
        return self._mean_metrics(logs)
    ...
    def fit(
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            ...
            print(f"[d3_epoch {epoch:03d}] train={train_stats}  valid={val_stats}")
