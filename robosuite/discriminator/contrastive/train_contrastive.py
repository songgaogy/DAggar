...
from robosuite.discriminator.contrastive.dataset import LatentDynamicsDataset
from robosuite.discriminator.contrastive.model import (
    ContrastiveContextEncoder,
    Encoder,
    PureContrastiveModel,
)
from robosuite.discriminator.contrastive.trainer import Trainer, TrainerConfig
...
@hydra.main(version_base="1.2", config_path="./config", config_name="train_contrastive")
def main(cfg: DictConfig) -> None:
    ...
    if str(cfg.save_name) in ("", "null", "None"):
        save_name = f"pure_contrastive_{_now_tag()}.pt"
    else:
