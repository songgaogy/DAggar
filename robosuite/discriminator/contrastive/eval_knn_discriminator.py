...
from robosuite.discriminator.contrastive.knn_discriminator import (
...
def main(cfg: DictConfig) -> None:
    ...
    ckpt_path = _resolve_ckpt(str(cfg.model.checkpoint_path))
    print(f"Using pure contrastive checkpoint: {ckpt_path}")
    ...
    experts = load_policy_trajectories(
        ...
        max_trajectories=(
            None
            if int(cfg.data.max_expert_trajectories) <= 0
            else int(cfg.data.max_expert_trajectories)
        ),
    )
    ...
    fails = load_policy_trajectories(
        ...
        max_trajectories=(
            None
            if int(cfg.data.max_fail_trajectories) <= 0
            else int(cfg.data.max_fail_trajectories)
        ),
    )
    ...
    expert_bank, expert_holdout = split_train_val(
        experts,
        val_ratio=float(cfg.split.holdout_ratio),
        seed=int(cfg.seed),
    )
    ...
    expert_calib, expert_eval = split_train_val(
        expert_holdout,
        val_ratio=float(cfg.split.calib_ratio_within_holdout),
        seed=int(cfg.seed) + 1,
    )
    ...
    extractor = PureContrastiveFeatureExtractor(
    ...
    def _encode_set(trajs):
        return [extractor.encode_trajectory(tr) for tr in trajs]

    bank_features = _encode_set(expert_bank)
    calib_features = _encode_set(expert_calib)
    eval_features = _encode_set(expert_eval)
    ...
    for feat in eval_features:
        out = detector.detect_sequence(feat, labels=None, adaptive_delta=False, aux_scores=None)
    ...
    fail_metrics["threshold_final"] = float(
        detector.threshold if detector.threshold is not None else np.nan
    )
    ...
    expert_metrics = {
        ...
        "false_alarm_rate": (
            float(expert_false_alarm / expert_total) if expert_total > 0 else float("nan")
        ),
        "lambda_expert_mean": (
            float(np.mean(expert_lambda_values)) if expert_lambda_values else float("nan")
        ),
    }
    ...
    lambda_stats = {
        ...
        "lambda_prefail_mean": (
            float(np.mean(pre_fail_lambda_values)) if pre_fail_lambda_values else float("nan")
        ),
        "lambda_gap": (
    ...
    tag = _now_tag()
    summary_path = os.path.join(save_dir, f"pure_contrastive_knn_summary_{tag}.json")
