...
# Typical usage:
#   BENCHMARK_JSON=checkpoints/dyn_disc/real_world_eval/run_20260429_103014/benchmark.json \
#     bash robosuite/discriminator/dyn_disc/scripts/vis_realworld_latent.sh
...
EVAL_ROOT="${EVAL_ROOT:-${REPO_ROOT}/checkpoints/dyn_disc/real_world_eval}"
...
if [[ ! -f "${MODEL_CKPT}" ]]; then
    echo "[real_world][dyn_disc][latent] ERROR: MODEL_CKPT not found: ${MODEL_CKPT}" >&2
    exit 1
fi
...
"${PYTHON_BIN}" -m robosuite.discriminator.dyn_disc.utils.vis_latent \
...
echo "[real_world][dyn_disc][latent] wrote to ${OUT_DIR}"
