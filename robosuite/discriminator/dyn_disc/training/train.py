"""Single-GPU Hydra training entry for the dyn_disc DINOv3 dynamics model.
...
This is a stripped-down rewrite of `dyn_model/train.py` that:
  ...
  * builds the WAM model architecture (DINOv3 encoder + hydra-instantiated
...
def _instantiate_encoder(cfg: DictConfig):
    ...
    if encoder_cfg is None:
        raise ValueError(
            "Missing `encoder` config. dyn_disc is DINOv3-only; set "
            "config/encoder/dinov3.yaml explicitly."
        )
