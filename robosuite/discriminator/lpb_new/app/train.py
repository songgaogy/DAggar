...
from robosuite.discriminator.lpb_new.core.model import LatentDynamicsModel, build_latent_dynamics_predictor
from robosuite.discriminator.lpb_new.core.trainer import Trainer, TrainerConfig
...
def run_train(cfg: DictConfig) -> None:
    ...
    try:
        ...
        if str(cfg.save_name) in {"", "None", "null"}:
            save_name = f"lpb_new_{now_tag()}.pt"
        else:
        ...
        def _save_periodic(epoch: int, history: dict[str, dict[str, dict[str, float]]]) -> None:
            # Keep periodic checkpoints lightweight: same payload format as the final save.
