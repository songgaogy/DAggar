...
SAVE_DIR="${SAVE_DIR:-${ROOT}/checkpoints/multitask_6/lpb_new/eval}"
...
if [[ ! -f "${CKPT}" ]]; then
  echo "[eval_lpb_new_knn] Missing policy checkpoint: ${CKPT}" >&2
  exit 1
fi

if [[ -z "${LPB_CKPT}" || ! -f "${LPB_CKPT}" ]]; then
  echo "[eval_lpb_new_knn] Missing LPB checkpoint. Set LPB_CKPT=/abs/path/to/lpb_new_XXXX.pt" >&2
  exit 1
fi

export CUDA_VISIBLE_DEVICES=0

echo "[eval_lpb_new_knn] ROOT=${ROOT}"
echo "[eval_lpb_new_knn] policy.ckpt=${CKPT}"
echo "[eval_lpb_new_knn] model.lpb_ckpt=${LPB_CKPT}"

"${PYTHON_BIN}" "${ROOT}/robosuite/discriminator/lpb_new/eval_knn_discriminator.py" \
  seed="${SEED}" \
  save_dir="${SAVE_DIR}" \
  policy.ckpt="${CKPT}" \
