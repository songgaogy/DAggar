"""Single-GPU Hydra training entry for the original LPB dynamics model on HDF5.

This is a stripped-down rewrite of `dyn_model/train.py` that:
  * reads HDF5 (via robosuite.discriminator.lpb_original.datasets.HDF5DynamicsModelDataset)
  ...
  * keeps the original LPB model architecture (ResNetEncoder + hydra-instantiated
    ...
    are loadable by `dyn_model.plan.load_model` and the KNN discriminator.
...
# Trigger sys.path injection (`import dyn_model`, `import diffusion_policy`).
...
def _build_model(cfg: DictConfig, dataset, device: torch.device):
    """Replicates the model construction in dyn_model/train.py.init_models."""
    encoder = ResNetEncoder(policy_ckpt_path=cfg.policy_ckpt_path, view_names=list(cfg.view_names))
...
@hydra.main(config_path="conf", config_name="train_hdf5", version_base=None)
def main(cfg: DictConfig) -> None:
    ...
    with open_dict(cfg):
        ...
        # Save the actual proprio/action input dims so plan.load_model can
        # reconstruct the encoder correctly without relying on path-name heuristics.
