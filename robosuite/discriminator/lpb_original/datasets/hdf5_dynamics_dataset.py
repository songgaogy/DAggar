"""HDF5 dataset adapter for the original LPB dynamics-model training loop.
...
# Trigger sys.path injection so `from diffusion_policy....` resolves.
...
class HDF5DynamicsModelDataset(Dataset):
    ...
    def __init__(
        ...
        for fp in files:
            with h5py.File(fp, "r") as f:
                ...
                for demo_key in sorted(f["demos"].keys()):
                    ...
                    if T <= 0:
                        continue
                    states_list.append(np.asarray(demo["states"][:T], dtype=np.float32))
        ...
        for start, end in zip(self.episode_start_indices, self.episode_end_indices):
            ...
            anchor_end = int(end) - self.num_pred * self.frameskip
            if anchor_end >= anchor_start:
                valid.extend(range(anchor_start, anchor_end))
