...
class Trainer:
    def __init__(self, cfg):
        ...
        self._warned_missing_language_encoder = False
    ...
    def init_models(self):
        ...
        if self.encoder is None:
            assert self.cfg.encoder_ckpt_path is None and self.train_encoder is False, (
                "When using original-LPB ResNetEncoder bootstrap/fallback, set "
                "`encoder_ckpt_path=null` and `model.train_encoder=false`."
            )
        ...
        language_emb_dim = 0
        if 'libero' in self.cfg.env.train_data_path:
            if self.cfg.env.policy_ckpt_path:
                ...
                log.warning(
                    "No env.policy_ckpt_path provided; disabling LanguageEncoder (libero text features will be skipped)."
                )
        ...
        for i, data in enumerate(
            ...
            if 'language' in obs and obs['language'] is not None:
                if self.language_encoder is not None:
                    ...
                    if not self._warned_missing_language_encoder:
                        log.warning(
                            "Batch contains language observations but LanguageEncoder is disabled (no policy_ckpt_path). "
                            "Skipping text feature extraction."
                        )
                        self._warned_missing_language_encoder = True
                    obs["language"] = None
