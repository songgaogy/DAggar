...
class LanguageEncoder(torch.nn.Module):
    def __init__(self, policy_ckpt_path: Optional[str]):
        super().__init__()
        self.policy_ckpt_path = policy_ckpt_path

        if not self.policy_ckpt_path:
            raise ValueError("LanguageEncoder requires policy_ckpt_path (diffusion-policy checkpoint).")
