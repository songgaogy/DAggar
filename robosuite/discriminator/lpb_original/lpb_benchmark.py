"""Benchmark adapter for the original LPB KNN OOD discriminator.

Drop-in alternative to `robosuite.discriminator.lpb.lpb_benchmark` but built on
the original LPB dynamics model + KNN feature definition (see `discriminator.py`).
...
import robosuite.discriminator.lpb_original  # noqa: F401  (sys.path injection)
...
from .discriminator import LPBOriginalEncoder, LPBOriginalKNN
...
class LPBOriginalBenchmarkDiscriminator:
    """Original LPB KNN OOD detector exposed through BenchmarkTrajectory API."""

    name = "lpb_original_knn"
    ...
    def __init__(
        ...
        *,
        ...
        self.encoder = LPBOriginalEncoder(model_ckpt=self.model_ckpt, device=self.device)
        ...
        # Per-task state.
        self._detectors_per_task: Dict[str, LPBOriginalKNN] = {}
    ...
    def _encode(self, trajectory: BenchmarkTrajectory) -> torch.Tensor:
        key = (str(trajectory.file_path), str(trajectory.demo_path))
