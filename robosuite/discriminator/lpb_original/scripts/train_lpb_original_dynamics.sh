#!/usr/bin/env bash
# Training entry for the original-LPB dynamics model on user's HDF5 data.
# Run from repo root:
#   TASK=PickPlaceCan POLICY_CKPT=/abs/path/to/dp_policy.ckpt \
...
#   env.policy_ckpt_path -> $POLICY_CKPT (optional)
...
LOAD_ALL_INTO_RAM="${LOAD_ALL_INTO_RAM:-1}"
...
if [[ -n "${POLICY_CKPT}" ]]; then
    EXTRA_OVERRIDES+=("env.policy_ckpt_path=${POLICY_CKPT}")
fi
