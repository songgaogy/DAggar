...
# Two-bank knobs.
FAIL_BANK_PER_TASK="${FAIL_BANK_PER_TASK:-10}"
# Keep all frames after fail label in the fail bank.
FAIL_BANK_ALL_AFTER_LABEL_K=1000000000
...
"${PYTHON_BIN}" -m robosuite.discriminator.lpb_v2.real_world_two_bank \
    ...
    --fail-bank-last-k    "${FAIL_BANK_ALL_AFTER_LABEL_K}" \
...
echo "[real_world][two_bank] wrote ${SAVE_JSON}"
