...
@dataclass
class MultiViewPolicyTrajectory:
    """Trajectory container for multiview policy latent extraction."""
    ...
    def __post_init__(self) -> None:
        ...
        self.images = np.asarray(self.images)
        if self.states.ndim != 2:
            raise ValueError(f"MultiViewPolicyTrajectory.states must be (T, D), got shape={self.states.shape}")
        if self.images.ndim != 5:
            raise ValueError(f"MultiViewPolicyTrajectory.images must be (T, V, H, W, C), got shape={self.images.shape}")
        if self.images.shape[-1] != 3:
            raise ValueError(f"MultiViewPolicyTrajectory.images must have 3 channels, got shape={self.images.shape}")
        if self.states.shape[0] != self.images.shape[0]:
            raise ValueError(
                "states and images length mismatch: "
                f"states={self.states.shape[0]} images={self.images.shape[0]}"
        ...
        if self.actions is not None:
            self.actions = np.asarray(self.actions)
            if self.actions.ndim != 2:
                raise ValueError(f"MultiViewPolicyTrajectory.actions must be (T, A), got shape={self.actions.shape}")
            if self.actions.shape[0] != self.states.shape[0]:
                raise ValueError(
                    "actions length mismatch: "
                    f"actions={self.actions.shape[0]} states={self.states.shape[0]}"
...
def load_multiview_policy_trajectory_from_ref(
    ref: TrajectoryRef,
    camera_names: list[str],
) -> MultiViewPolicyTrajectory:
    """
    Load one trajectory containing state + multi-camera images for policy-latent encoding.
    """
    require_h5py()
    cameras = [str(name) for name in camera_names]
    if not cameras:
        raise ValueError("camera_names must be non-empty for multiview loading")
    ...
    with h5py.File(ref.file_path, "r") as f:
        ...
        for camera_name in cameras:
            ...
            lengths.append(int(images.shape[0]))
    ...
    stacked_images = np.stack([x[:t] for x in image_views], axis=1)
    return MultiViewPolicyTrajectory(
        states=states[:t],
...
def load_multiview_policy_trajectories(
    data_dir: str,
    camera_names: list[str],
    max_trajectories: Optional[int] = None,
) -> list[MultiViewPolicyTrajectory]:
    refs = scan_hdf5_trajectories(data_dir=data_dir, max_trajectories=max_trajectories)
    trajectories: list[MultiViewPolicyTrajectory] = []
    for ref in refs:
        try:
            trajectories.append(load_multiview_policy_trajectory_from_ref(ref=ref, camera_names=camera_names))
        except Exception:
            continue
    if not trajectories:
        raise RuntimeError(
            f"No valid multiview policy trajectories found in {data_dir} for cameras {camera_names}."
        )
    return trajectories
