...
try:
    ...
    try:
        from robosuite.policy.flow_v0.flow import FlowPolicy, sinusoidal_time_embedding
        from robosuite.policy.flow_v0.utils.env_util import PandaLiftProprioExtractor
    except ModuleNotFoundError:
        if exc.name not in {"mujoco", "robosuite"}:
            raise
        # Fallback import path for environments without robosuite top-level package deps.
        from policy.flow_v0.flow import FlowPolicy, sinusoidal_time_embedding
        from policy.flow_v0.utils.env_util import PandaLiftProprioExtractor

try:
    from robosuite.policy.flow_unet.model import build_flow_policy
    from robosuite.policy.flow_unet.utils.env_util import RobosuiteProprioExtractor
except ModuleNotFoundError as exc:  # pragma: no cover
    if exc.name not in {"mujoco", "robosuite"}:
        raise
    from policy.flow_unet.model import build_flow_policy
    from policy.flow_unet.utils.env_util import RobosuiteProprioExtractor

from .float_data import MultiViewPolicyTrajectory, PolicyTrajectory
...
class FlowUNetPolicyLatentExtractor:
    """
    Extract policy observation embeddings from the current flow_unet checkpoint.

    Embedding phi(o_t) is the fused context token returned by `encode_context`.
    """
    ...
    def __init__(
        ...
        if "camera_names" not in ckpt:
            raise KeyError(f"Checkpoint missing `camera_names`: {ckpt_path}")
        if "env_metadata" not in ckpt:
            raise KeyError(f"Checkpoint missing `env_metadata`: {ckpt_path}")

        self.camera_names = [str(name) for name in ckpt["camera_names"]]
        if not self.camera_names:
            raise ValueError("flow_unet checkpoint has empty camera_names")

        self.env_metadata = dict(ckpt["env_metadata"])
        self.prop_mean = ckpt.get("prop_mean")
        self.prop_std = ckpt.get("prop_std")
        ...
        if self.prop_mean is not None:
            self.prop_mean = np.asarray(self.prop_mean, dtype=np.float32)
        if self.prop_std is not None:
            self.prop_std = np.asarray(self.prop_std, dtype=np.float32)

        model_cfg = ckpt.get("model_cfg")
        if model_cfg is None:
            cfg = ckpt.get("cfg", {})
            if isinstance(cfg, dict):
                model_cfg = cfg.get("flow")
        if model_cfg is None:
            raise KeyError(f"Checkpoint missing `model_cfg` / `cfg.flow`: {ckpt_path}")

        act_mean = ckpt.get("act_mean")
        if act_mean is None:
            raise KeyError(f"Checkpoint missing `act_mean`: {ckpt_path}")
        if self.prop_mean is None:
            raise KeyError(f"Checkpoint missing `prop_mean`: {ckpt_path}")

        action_dim = int(np.asarray(act_mean).shape[-1])
        proprio_dim = int(self.prop_mean.shape[-1])

        cfg = ckpt.get("cfg", {})
        data_cfg = cfg.get("data", {}) if isinstance(cfg, dict) else {}
        ckpt_image_size = int(data_cfg.get("image_size", 128))
        self.image_size = ckpt_image_size if int(image_size) <= 0 else int(image_size)
        ...
        state_dict = ckpt.get("ema_model", ckpt["model"]) if bool(use_ema) else ckpt["model"]
        self.model.load_state_dict(state_dict, strict=True)
        self.model.eval()

        self.img_normalize = Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        self.proprio_extractor = RobosuiteProprioExtractor(
            env_kwargs=self.env_metadata,
            has_renderer=False,
            has_offscreen_renderer=False,
            use_camera_obs=False,
            camera_names=None,
    ...
    def _prepare_images(self, images_raw: np.ndarray) -> np.ndarray:
        if images_raw.ndim != 5:
            raise ValueError(f"Expected multiview images shape (T,V,H,W,C), got {images_raw.shape}")
        t_len, num_views = int(images_raw.shape[0]), int(images_raw.shape[1])
        if num_views != len(self.camera_names):
            raise ValueError(
                f"Image view mismatch: expected {len(self.camera_names)} views, got {num_views}"
            )

        images = np.empty((t_len, num_views, 3, self.image_size, self.image_size), dtype=np.float32)
        for t in range(t_len):
            for view_idx in range(num_views):
                img = _center_crop_resize(images_raw[t, view_idx], self.image_size)
                images[t, view_idx] = np.transpose(img.astype(np.float32) / 255.0, (2, 0, 1))
        return images
    ...
    def encode_trajectory_with_indices(self, traj: MultiViewPolicyTrajectory) -> tuple[np.ndarray, np.ndarray]:
        ...
        for start in range(0, images.shape[0], self.batch_size):
            ...
            img_batch = torch.from_numpy(images[start:stop]).to(self.device)
            prop_batch = torch.from_numpy(proprio[start:stop]).to(self.device)

            b, v, c, h, w = img_batch.shape
            img_flat = img_batch.view(b * v, c, h, w)
            img_norm = self.img_normalize(img_flat).view(b, v, c, h, w)

            emb = self.model.encode_context(images=img_norm, proprio=prop_batch)
            outputs.append(emb.detach().cpu().numpy().astype(np.float32))

        return np.concatenate(outputs, axis=0), sampled_indices

    @torch.no_grad()
    def encode_trajectory(self, traj: MultiViewPolicyTrajectory) -> np.ndarray:
        emb, _ = self.encode_trajectory_with_indices(traj)
        return emb
