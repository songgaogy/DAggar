...
class Trainer:
    ...
    def _run_step(self, batch: dict[str, torch.Tensor], train: bool) -> dict[str, float]:
        ...
        if "latent_nll" in stats:
            out["latent_nll"] = float(stats["latent_nll"].detach().item())
        if "pred_logvar" in stats:
            out["logvar_mean"] = float(stats["pred_logvar"].detach().mean().item())
        return out
