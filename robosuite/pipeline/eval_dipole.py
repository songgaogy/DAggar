...
from robosuite.pipeline.train_dipole import convert_env_camera_observation
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a dipole checkpoint and optionally save rollout videos.")
    parser.add_argument("--checkpoint", required=True, help="Path to a dipole checkpoint.")
    parser.add_argument("--env-name", required=True, help="Robosuite env name, e.g. PandaStack or PickPlaceBread.")
...
def _resolve_eval_device(payload: dict[str, Any]) -> str:
    dipole_cfg = dict(payload.get("dipole_config", {}))
    requested = dipole_cfg.get("inference_device") or dipole_cfg.get("device") or "cpu"
...
def _build_agent(
    ...
    *,
    ...
    algorithm_cfg = {
        ...
        "dipole": dict(checkpoint_payload.get("dipole_config", {})),
        "online_buffer": {"capacity": 1},
        "demo_buffer": {"capacity": 1},
        "trainer": dict(checkpoint_payload.get("trainer_config", {})),
    }
    algorithm_cfg["dipole"]["model"] = dict(checkpoint_payload["model_cfg"])
    algorithm_cfg["dipole"]["device"] = str(device)
    algorithm_cfg["dipole"]["inference_device"] = str(device)
...
def main() -> None:
    ...
    image_size = int(dict(checkpoint_payload.get("dipole_config", {})).get("image_size", 128))
    ...
    try:
        ...
        summary = {
            ...
            "dipole_config": dict(checkpoint_payload.get("dipole_config", {})),
