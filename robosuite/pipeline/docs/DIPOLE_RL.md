...
- **Q-chunk input is action-sensitive (EXPO-FT-style)**: context is squashed `LayerNorm → tanh` (bounds the unbounded encoder latent to [-1,1]); the action chunk is z-scored (per-dim `action_mean`/`action_std` buffers) then projected `Linear → LayerNorm → tanh` to a 256-d embedding; Q MLP input is `D_ctx + 256`. V input is `D_ctx`. No per-step Q factorization.
...
- `robosuite/pipeline/algorithms/q_learning/networks.py` — `_build_mlp` with `Linear → LayerNorm → GELU` blocks, Kaiming-normal hidden init, **zero-init final layer (weight + bias)**. `QChunkNetwork` squashes context (`LayerNorm → tanh`) and projects the z-scored action chunk (`Linear → LayerNorm → tanh`, Kaiming-init — **NOT** zero-init) to a 256-d embedding, so MLP input dim is `D_ctx + 256`; per-action-dim `action_mean`/`action_std` are registered buffers (default identity, populated in warmup, round-trip via `state_dict`). `VNetwork` input dim `D_ctx`.
