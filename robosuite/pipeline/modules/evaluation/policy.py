...
Reuses the shared evaluation helpers (policy build, frame capture, MP4 writer)
so the offline-finetuned checkpoint is evaluated exactly like an online DIPOLE
checkpoint, with DIPOLE two-branch guidance
``(1 + omega) * v_pos - omega * v_neg``.
...
Example::
    ...
    MUJOCO_GL=egl python -m robosuite.pipeline.modules.evaluation.policy \\
...
from robosuite.pipeline.common.environment import build_robosuite_env, sparse_success_reward
from robosuite.pipeline.modules.evaluation.policy_runtime import (
    DEFAULT_EVAL_SEED,
