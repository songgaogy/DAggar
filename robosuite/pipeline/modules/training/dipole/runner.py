"""Offline DIPOLE training runner."""
...
from robosuite.pipeline.algorithms.vast.checkpoint import load_vast_weights
...
from robosuite.pipeline.modules.visualization.training_diagnostics import (
    plot_branch_weight_distribution,
    plot_raw_g_distribution,
    plot_v_pos_neg_scatter,
)
from robosuite.pipeline.modules.training.dipole import (
...
def _resolve_episodes_path(cfg: DictConfig, task_name: str) -> str:
    ...
    if not default.exists():
        raise FileNotFoundError(
            ...
            "or run the collection stage first."
        )
    return str(default)
...
def _warmup_transitions_path(cfg: DictConfig, task_name: str) -> str:
    explicit = OmegaConf.select(
        cfg, "offline.vast_warmup_transitions_path", default=None
    )
    if explicit is not None and str(explicit).strip().lower() not in {"", "null"}:
        return _resolve_required_path(
            explicit,
            what="offline.vast_warmup_transitions_path",
        )
    data_root = str(cfg.offline.data_root)
...
def _resolve_episodes_paths(cfg: DictConfig, task_name: str) -> list[str]:
    raw_paths = OmegaConf.select(cfg, "offline.episodes_paths", default=None)
    if raw_paths is None:
        return [_resolve_episodes_path(cfg, task_name)]
    paths = [
        _resolve_required_path(value, what="offline.episodes_paths")
        for value in list(raw_paths)
    ]
    if not paths:
        raise ValueError("offline.episodes_paths must not be empty.")
    return paths
...
def run_offline_training(cfg: DictConfig) -> None:
    torch.set_float32_matmul_precision("high")
    execution_stage = str(
        OmegaConf.select(cfg, "offline.execution_stage", default="all")
    ).strip().lower()
    if execution_stage not in {"all", "vast", "policy"}:
        raise ValueError(
            "offline.execution_stage must be one of all, vast, or policy, "
            f"got {execution_stage!r}."
        )
    ...
    if execution_stage == "vast" and skip_rl:
        raise ValueError("The VAST stage cannot run with offline.skip_rl=true.")
    if execution_stage == "policy" and not skip_rl:
        raise ValueError(
            "The policy stage requires offline.skip_rl=true and a completed VAST checkpoint."
        )
    ...
    # ------------------------------------------------------------------ #
    episodes_paths = _resolve_episodes_paths(cfg, task_name)
    ...
    if execution_stage == "vast":
        write_run_info(
            run_dir,
            {
                ...
                "task_name": task_name,
                "discriminator_checkpoint": nnpu_ckpt,
                "input_vast_checkpoint": str(initial_vast_ckpt),
        ...
        if metric_logger is not None:
            metric_logger.close()
        print(f"[offline][vast] stage complete -> {vast_ckpt_path}")
        return
    ...
    def _save(tag: str, step: int) -> Path:
        payload = agent.build_checkpoint_payload(
            ...
            parent_payload=ctx.init_payload,
        )
