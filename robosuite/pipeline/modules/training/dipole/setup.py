...
The batch-online VAST/policy stages use
...
from robosuite.pipeline.common.environment import build_robosuite_env
...
def build_agent_env(cfg: DictConfig, *, log_tag: str = "offline") -> AgentEnvContext:
    ...
    agent.load_policy_checkpoint(init_checkpoint, task_name=task_name)
    print(f"[{log_tag}] loaded policy weights from {init_checkpoint}")
...
def finalize_normalizers(
    ...
    *,
    ...
    if agent.replay_buffer.num_valid_sequences() < batch_size:
        raise RuntimeError(
            f"replay_buffer has only {agent.replay_buffer.num_valid_sequences()} valid "
