...
Loads disk sources into one batch policy replay buffer (`agent.replay_buffer`), fully
...
def load_pretrain_transitions(
    *,
    ...
    same closure collection/warmup build around
...
def populate_replay_buffer(
    ...
    """Add ``transitions`` to ``buffer`` (typically ``agent.replay_buffer``).
