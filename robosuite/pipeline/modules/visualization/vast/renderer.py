...
from robosuite.pipeline.common.flow import load_hdf5_demos_into_flow_transitions
...
# Must match the resolved environment image height when a checkpoint omits it.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", default="outputs/vast-warmup/vast-cache-vis")
...
def resolve_device(requested: str | None, checkpoint_device: str) -> str:
    device = str(requested or checkpoint_device or "cuda:0")
    if not device.startswith("cuda"):
        raise RuntimeError(f"VAST visualization requires CUDA, got device={device}")
...
def main() -> None:
    ...
    device = resolve_device(args.device, str(payload["cfg"].get("device", "cuda:0")))
