...
class FlowDaggerTrainer:
    def __init__(self, agent: FlowDaggerAgent, config=None) -> None:
        self.agent = agent
    ...
    def train_step(self, batch_size: int | None = None) -> dict[str, float]:
        if not self.agent.ready_for_update(batch_size=batch_size):
            raise RuntimeError("Not enough valid flow-dagger demo sequences are available for an update.")
    ...
    def start_async_worker(self) -> None:
        ...
        with self._async_condition:
            if self._async_thread is not None:
                return
            self._async_stop_requested = False
            self._async_thread = threading.Thread(target=self._async_update_loop, name="flow_dagger_learner", daemon=True)
    ...
    def _raise_async_error(self) -> None:
        if self._async_error is not None:
            raise RuntimeError(f"Async flow-dagger learner failed: {self._async_error}") from self._async_error
