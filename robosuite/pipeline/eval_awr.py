...
DEFAULT_OUTPUT_ROOT = "./outputs/awr/eval"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate an AWR checkpoint and optionally save rollout videos.")
    parser.add_argument("--checkpoint", required=True, help="Path to an AWR checkpoint.")
    parser.add_argument("--env-name", required=True, help="Robosuite env name, e.g. Stack or PickPlaceCan.")
    parser.add_argument("--task-name", required=True, help="Task name used for env metadata resolution.")
    ...
    parser.add_argument("--device", default=None, help="Optional override for policy eval device.")
    parser.add_argument("--inference-device", default=None, help="Optional alias for --device.")
...
def _resolve_eval_device(payload: dict[str, Any], requested: str | None) -> str:
    awr_cfg = dict(payload.get("awr_config", {}))
    device_request = requested or awr_cfg.get("inference_device") or awr_cfg.get("device") or "cpu"
    fallback = "cuda:0" if torch.cuda.is_available() else "cpu"
    return resolve_requested_device(device_request, fallback=fallback)
...
def _build_agent(
    ...
    *,
    task_name: str,
    device: str,
    observation_example: dict[str, Any],
    ...
    algorithm_cfg = {
        "type": "awr",
        "task_name": str(checkpoint_payload.get("task_name", task_name)),
        "camera_names": [str(name) for name in checkpoint_payload["camera_names"]],
        "encoder": dict(checkpoint_payload.get("encoder_config", {})),
        "awr": dict(checkpoint_payload.get("awr_config", {})),
        "online_buffer": {"capacity": 1},
        "demo_buffer": {"capacity": 1},
        "trainer": dict(checkpoint_payload.get("trainer_config", {})),
    }
    algorithm_cfg["awr"]["model"] = dict(checkpoint_payload["model_cfg"])
    algorithm_cfg["awr"]["device"] = str(device)
    algorithm_cfg["awr"]["inference_device"] = str(device)
    algorithm_cfg["awr"]["reward_from_discriminator"] = False
    ...
    agent.load_checkpoint(checkpoint_path, load_buffers=False)
...
def main() -> None:
    ...
    image_size = int(dict(checkpoint_payload.get("awr_config", {})).get("image_size", 128))
    ...
    runtime_cfg.env_name = str(args.env_name)
    ...
    try:
        ...
        initial_obs = convert_env_camera_observation(
            raw_obs,
            env=env,
        ...
        requested_device = args.device or args.inference_device
        eval_device = _resolve_eval_device(checkpoint_payload, requested_device)
        agent = _build_agent(
            checkpoint_path,
            checkpoint_payload,
        ...
        summary = {
            ...
            "awr_config": dict(checkpoint_payload.get("awr_config", {})),
