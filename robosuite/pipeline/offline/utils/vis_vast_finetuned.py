...
``robosuite.pipeline.algorithms.vast.utils.vis_vast``. Offline finetune
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "standard vast.utils.vis_vast visualizer."
        )
    )
...
def main() -> None:
    ...
    output_root = run_dir / "vis_vast"
    forwarded = drop_forwarded_option(forwarded, "--output-root")

    from robosuite.pipeline.algorithms.vast.utils import vis_vast

    original_loader = vis_vast.load_vast_payload
    ...
    vis_vast.load_vast_payload = patched_load_vast_payload
    old_argv = sys.argv[:]
    try:
        sys.argv = [
            old_argv[0],
        ...
        sys.argv = old_argv
        vis_vast.load_vast_payload = original_loader
