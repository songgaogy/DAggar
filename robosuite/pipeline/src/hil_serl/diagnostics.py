...
from robosuite.pipeline.src.data.demos import load_hdf5_demos_into_transitions
from robosuite.pipeline.src.data.transitions import Transition
from robosuite.pipeline.src.hil_serl.agent import HILSERLAgent
from robosuite.pipeline.utils.tensor import nested_to_torch, require_cuda_device, stack_tree


DEFAULT_CHECKPOINT = "outputs/baseline/hil-serl/PickPlaceCereal/checkpoints/latest.pt"
DEFAULT_SUCCESS_DIR = "data/PickPlaceCereal/success_rollout"
DEFAULT_OUTPUT_ROOT = "outputs/baseline/hil-serl/PickPlaceCereal/qv_visualization"
...
def prepare_algorithm_cfg(payload: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    learner_device = str(sac_cfg.get("device", "cuda:0"))
    inference_device = str(sac_cfg.get("inference_device", learner_device))
    sac_cfg["device"] = require_cuda_device(learner_device)
    sac_cfg["inference_device"] = require_cuda_device(inference_device)
...
def compute_qv_metrics(agent, transitions: list[Transition], *, mc_action_samples: int) -> list[dict[str, float]]:
    ...
    grasp_penalty = torch.as_tensor(
        np.asarray(
            [
                0.0 if transition.grasp_penalty is None else float(transition.grasp_penalty)
...
def estimate_soft_value(core, features: torch.Tensor, *, samples: int, use_target_critic: bool = False) -> torch.Tensor:
    ...
    for _ in range(samples):
        sampled_actions, log_probs = core.actor.sample(
            features,
            temperature=core.alpha.detach(),
            deterministic=False,
        )
...
def summarize_metrics(metrics: list[dict[str, float]], gamma: float) -> dict[str, Any]:
    ...
    lower_bound = 0.0
    upper_bound = 1.0 / max(1e-6, 1.0 - float(gamma))
    ...
    return {
        ...
        "expected_binary_reward_value_range": [float(lower_bound), float(upper_bound)],
