...
@dataclass
class DipoleDiscriminatorLabel:
    global_step: int
    episode_index: int
...
class DipolePolicyWorker:
    ...
    def start(self) -> None:
        with self._condition:
            if self._thread is not None:
                return
            self._stop_requested = False
            self._thread = threading.Thread(target=self._worker_loop, name="dipole_policy_worker", daemon=True)
            self._thread.start()
        ...
        with self._condition:
            ...
            while request_id not in self._results:
                ...
                if end_time is not None:
                    ...
                    if remaining <= 0.0:
                        raise TimeoutError("Timed out while waiting for Dipole policy worker action.")
        ...
        with self._condition:
            ...
            while self._pending_commands or self._busy:
                ...
                if end_time is not None:
                    remaining = end_time - time.monotonic()
                    if remaining <= 0.0:
                        raise TimeoutError("Timed out while waiting for Dipole policy worker.")
                    self._condition.wait(timeout=min(0.1, remaining))
                else:
                    self._condition.wait(timeout=0.1)
        self._raise_error()
    ...
    def close(self) -> None:
        ...
        if thread.is_alive():
            raise RuntimeError("Dipole policy worker did not stop cleanly.")
        with self._condition:
            self._thread = None
        self._raise_error()
        ...
        while True:
            ...
            try:
                ...
                if command_type == "select_action":
                    ...
                    raise KeyError(f"Unsupported DipolePolicyWorker command: {command_type}")
    ...
    def _raise_error(self) -> None:
        if self._error is not None:
            raise RuntimeError(f"Dipole policy worker failed: {self._error}") from self._error
...
class DipoleDiscriminatorWorker:
    ...
    def __init__(self, cfg: Any, *, task_name: str) -> None:
        ...
        if not isinstance(self.runtime, OfflinePrefixOnlineDiscriminator):
            raise TypeError(
                "DipoleDiscriminatorWorker currently requires an OfflinePrefixOnlineDiscriminator "
                f"instance, got {type(self.runtime).__name__}."
            )
        self._condition = threading.Condition()
        self._thread: threading.Thread | None = None
        self._pending_commands: deque[dict[str, Any]] = deque()
        self._results: deque[DipoleDiscriminatorLabel] = deque()
        self._stop_requested = False
        self._busy = False
        self._error: BaseException | None = None
    ...
    def start(self) -> None:
        with self._condition:
            ...
            self._thread = threading.Thread(
                ...
                name="dipole_discriminator_worker",
                daemon=True,
            )
            self._thread.start()
        ...
        with self._condition:
            ...
            while self._pending_commands or self._busy:
                ...
                if end_time is not None:
                    remaining = end_time - time.monotonic()
                    if remaining <= 0.0:
                        raise TimeoutError("Timed out while waiting for Dipole discriminator worker.")
                    self._condition.wait(timeout=min(0.1, remaining))
                else:
                    self._condition.wait(timeout=0.1)
        self._raise_error()
    ...
    def close(self) -> None:
        ...
        if thread.is_alive():
            raise RuntimeError("Dipole discriminator worker did not stop cleanly.")
        with self._condition:
            self._thread = None
        try:
            self.runtime.close()
    ...
    def _handle_record_steps(self, commands: list[dict[str, Any]]) -> None:
        ...
        new_labels: list[DipoleDiscriminatorLabel] = []
    ...
    def _raise_error(self) -> None:
        if self._error is not None:
            raise RuntimeError(f"Dipole discriminator worker failed: {self._error}") from self._error
