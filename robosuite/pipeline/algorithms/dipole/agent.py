...
class DipoleAgent:
    def __init__(
        ...
        *,
        ...
        self.dipole_config = dipole_config
        self.model_cfg = copy.deepcopy(model_cfg)
        self.camera_names = [str(name) for name in camera_names]
        self.task_name = str(dipole_config.task_name or "task")
        self.language_instruction = str(dipole_config.language_instruction or self.task_name)
    ...
    def from_config(
        ...
        *,
        ...
        encoder_config = EncoderConfig(
            ...
            proprio_keys=("state",),
            image_size=int(cfg_get(dipole_cfg, "image_size", 128)),
        )
        model_cfg = copy.deepcopy(cfg_get(dipole_cfg, "model", {}))
        augmentation_cfg = cfg_get(dipole_cfg, "augmentation", None)
        batch_size = int(cfg_get(trainer_cfg, "batch_size", cfg_get(cfg, "batch_size", 64)))
        inference_device = cfg_get(dipole_cfg, "inference_device", None)
        if inference_device is not None and str(inference_device).strip().lower() == "auto":
            inference_device = None

        task_name = str(cfg_get(cfg, "task_name", "task"))
        task_prompt_map = cfg_get(dipole_cfg, "task_prompt_map", None)
        resolved_device = str(device or cfg_get(dipole_cfg, "device", cfg_get(cfg, "device", "cpu")))
        dipole_config = DipoleConfig(
            action_dim=action_dim,
            proprio_dim=proprio_dim,
            action_horizon=int(cfg_get(dipole_cfg, "action_horizon", 8)),
            execute_horizon=int(cfg_get(dipole_cfg, "execute_horizon", 1)),
            image_size=int(cfg_get(dipole_cfg, "image_size", 128)),
            learning_rate=float(cfg_get(dipole_cfg, "learning_rate", 1e-4)),
            weight_decay=float(cfg_get(dipole_cfg, "weight_decay", 1e-6)),
            grad_clip_norm=float(cfg_get(dipole_cfg, "grad_clip_norm", 1.0)),
    ...
    def fit_normalizers_from_transitions(self, transitions: list[Transition]) -> None:
        ...
        action_windows = []
        proprio_values = []
        horizon = int(self.dipole_config.action_horizon)
    ...
    def patch_online_transition_label(
        self,
        *,
        episode_index: int,
        episode_step: int,
        ...
        return self.online_buffer.patch_dipole_label(
    ...
    def sample_online_batch(self, batch_size: int | None = None):
        batch_size = int(batch_size or self.trainer_config.batch_size)
    ...
    def sample_mixed_batch(self, batch_size: int | None = None):
        ...
        if demo_valid <= 0 and online_valid <= 0:
            raise RuntimeError("Dipole agent has no valid sequences available for training.")

        target_online = int(round(batch_size * float(self.trainer_config.online_fraction)))
        target_online = min(max(0, target_online), batch_size)
        if online_valid <= 0:
            target_online = 0
        if demo_valid <= 0:
            target_online = batch_size
        online_batch_size = int(target_online)
        demo_batch_size = int(batch_size - online_batch_size)
        batches = []
        if demo_batch_size > 0:
            batches.append(self.sample_demo_batch(batch_size=demo_batch_size))
        if online_batch_size > 0:
            batches.append(self.sample_online_batch(batch_size=online_batch_size))
        from .common import DipoleBatch

        return DipoleBatch.concat(batches)
    ...
    def build_checkpoint_payload(
        ...
        *,
        ...
        payload = {
            ...
            "dipole_config": asdict(self.dipole_config),
