...
from ..common.types import EncoderConfig
from ..common.utils import standardize_image_tensor
...
class OfficialPretrainedResNet10Encoder(nn.Module):
    ...
    def _infer_backbone_shape(self) -> tuple[int, int, int]:
        ...
        return int(feature_map.shape[-2]), int(feature_map.shape[-1]), int(feature_map.shape[1])
...
class MultiModalObservationEncoder(nn.Module):
    def __init__(self, observation_example, config: EncoderConfig) -> None:
        ...
        if not self.image_keys:
            self.image_keys = [
                key for key, value in example_dict.items() if _looks_like_image(torch.as_tensor(value))
            ]
        if not self.proprio_keys:
            self.proprio_keys = [key for key in example_dict.keys() if key not in self.image_keys]

        self.image_example_tensors = {
            key: standardize_image_tensor(torch.as_tensor(example_dict[key])) for key in self.image_keys
        }
    ...
    def _build_image_encoder(self, key: str) -> nn.Module:
        image = self.image_example_tensors[key]
...
def _looks_like_image(value: torch.Tensor) -> bool:
    if value.ndim < 3:
        return False
    spatial_shape = tuple(value.shape[-3:])
