...
class HGDaggerTrainer:
    def __init__(self, agent: HGDaggerAgent, config=None) -> None:
        self.agent = agent
    ...
    def train_step(self, batch_size: int | None = None) -> dict[str, float]:
        if not self.agent.ready_for_update(batch_size=batch_size):
            raise RuntimeError("Not enough demo transitions in the HG-DAgger buffer for an update.")
    ...
    def start_async_worker(self) -> None:
        ...
        with self._async_condition:
            if self._async_thread is not None:
                return
            self._async_stop_requested = False
            self._async_thread = threading.Thread(target=self._async_update_loop, name="hg_dagger_learner", daemon=True)
