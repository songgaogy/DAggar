...
TRAIN_EPISODE_MAX_STEPS="${TRAIN_EPISODE_MAX_STEPS:-400}"
AWR_MODE="${AWR_MODE:-train}"   # eval or train

EXPERT_NUM_TRAJ="${EXPERT_NUM_TRAJ:-20}"
SUCCESS_NUM_TRAJ="${SUCCESS_NUM_TRAJ:-180}"
FAIL_NUM_TRAJ="${FAIL_NUM_TRAJ:-180}"
NUM_TRAIN_STEP="${NUM_TRAIN_STEP:-300}"
...
case "${AWR_MODE}" in
  ...
  train)
    ...
    ;;
  *)
    echo "[ERROR] Unsupported AWR_MODE='${AWR_MODE}'. Use 'eval' or 'train'." >&2
    exit 1
    ;;
esac
...
python -m robosuite.pipeline.train_awr \
  seed="${SEED_VALUE}" \
...
# Example:
# AWR_MODE=train bash robosuite/pipeline/scripts/train_awr.sh
