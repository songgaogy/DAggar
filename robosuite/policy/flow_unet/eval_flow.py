...
from robosuite.policy.flow_unet.utils.env_util import RobosuiteProprioExtractor, camera_obs_key
...
@torch.no_grad()
def sample_action_sequence(model, images, proprio, action_horizon: int, n_steps: int):
...
def main(cfg: DictConfig):
    ...
    extractor = RobosuiteProprioExtractor(
        env_kwargs=checkpoint["env_metadata"],
        has_renderer=False,
    ...
    def rollout(record_video: bool = False):
        obs = env.reset()
        total_reward = 0.0
        success = False
        frames = []
        ...
        while step_count < int(cfg.eval.max_steps):
            ...
            for action in action_seq[:execute_steps]:
                ...
                if done or step_count >= int(cfg.eval.max_steps):
                    break
    ...
    for episode_idx in range(int(cfg.eval.episodes)):
        total_reward, step_count, success, frames = rollout(record_video=video_dir is not None)
        print(
            f"episode={episode_idx} return={total_reward:.4f} steps={step_count} success={int(success)}"
        )
        if video_dir is not None and len(frames) > 0:
            video_path = os.path.join(video_dir, f"episode_{episode_idx}_return_{total_reward:.2f}.mp4")
    ...
    if bool(cfg.eval.succ_rate):
        ...
        for _ in tqdm(range(num_eval_episodes), desc="Success-rate eval"):
            _, _, success, _ = rollout(record_video=False)
            success_count += int(success)
        print(
            f"checkpoint={cfg.eval.ckpt} success_rate={success_count / float(num_eval_episodes):.4f} "
