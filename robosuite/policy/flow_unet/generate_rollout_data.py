...
def main(cfg: DictConfig):
    ...
    output_path = os.path.join(
        output_dir,
        f"flow_unet_rollout_{keep_mode}_{base_time}_{int(cfg.generate.episodes)}.hdf5",
    )
    ...
    while saved_count < int(cfg.generate.episodes):
        ...
        while step_count < int(cfg.generate.max_steps) and not done:
            ...
            for action in action_seq[:execute_steps]:
                ...
                current_non_image_obs["flow_unet_proprio"] = extractor.extract(current_state).astype(np.float32)
