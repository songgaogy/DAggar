...
DEFAULT_TASK_PROMPTS = {
    ...
    "NutAssemblyRound": [
        "Pick the round nut and place it in the left round peg.",
    ],
    "NutAssemblySquare": [
        "Pick the square nut and place it in the right square peg.",
    ]
}
...
class RobosuiteMultiViewFlowDataset(Dataset):
    def __init__(
        ...
        num_train_traj: int | dict | None = None,
        seed: int = 0,
        ...
        self.num_train_traj = self._normalize_num_train_traj(num_train_traj)
        self.seed = int(seed)
    ...
    @staticmethod
    def _normalize_num_train_traj(num_train_traj):
        if num_train_traj is None:
            return None
        if isinstance(num_train_traj, int):
            return int(num_train_traj)
        mapping = _cfg_mapping_to_dict(num_train_traj)
        return {str(Path(key)): int(value) for key, value in mapping.items()}

    def _resolve_num_train_traj(self, data_dir: str):
        if self.num_train_traj is None:
            return None
        if isinstance(self.num_train_traj, int):
            return int(self.num_train_traj)
        return self.num_train_traj.get(str(Path(data_dir)))

    def _collect_candidate_demos(self, files: list[str], task_name: str):
        # Deterministic enumeration (sorted files -> sorted demo keys) so seeded sampling is reproducible.
        candidates = []
        for file_path in tqdm(files, desc=f"Scanning flow_multi demos [{task_name}]"):
            with h5py.File(file_path, "r") as handle:
                ...
                for demo_key in demo_keys:
                    ...
                    if num_steps < self.action_horizon:
                        continue
                    candidates.append((file_path, demo_key, num_steps))
        return candidates

    def _build_index(self):
        for dir_index, data_dir in enumerate(self.data_dirs):
            ...
            candidates = self._collect_candidate_demos(files, task_name)

            count = self._resolve_num_train_traj(data_dir)
            if count is not None and count < len(candidates):
                # Seeded random subset (per-dir seed) for reproducible sampling.
                rng = np.random.default_rng(self.seed + dir_index)
                chosen = sorted(int(i) for i in rng.choice(len(candidates), size=int(count), replace=False))
                selected_candidates = [candidates[i] for i in chosen]
            else:
                if count is not None and count > len(candidates):
                    print(
                        f"[flow_multi] task={task_name}: requested {count} demos but only "
                        f"{len(candidates)} valid demos available; using all."
                    )
                selected_candidates = candidates

            for file_path, demo_key, num_steps in selected_candidates:
        ...
        self.selected_demos = list(self._demo_meta)
