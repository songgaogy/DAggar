...
class BaseWorkspace:
    ...
    @property
    def output_dir(self):
        # Prefer explicit cfg.output_dir if provided
        if hasattr(self, "cfg") and (self.cfg is not None) and ("output_dir" in self.cfg):
            return os.path.abspath(self.cfg.output_dir)

        # Fallback to Hydra runtime if available
        try:
            return HydraConfig.get().runtime.output_dir
        except ValueError:
            # Final fallback when HydraConfig is not set
            return os.getcwd()
