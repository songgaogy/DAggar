#!/usr/bin/env bash
# LPB KNN benchmark entry point (ported skin under benchmark/robosuite/).
# Run from repo root:
#   bash benchmark/robosuite/scripts/run_lpb_benchmark.sh
...
"${PYTHON_BIN}" -m benchmark.robosuite.examples.run_lpb \
    --lpb-ckpt                        "${LPB_CKPT}" \
    --fail-root                       "${FAIL_ROOT}" \
